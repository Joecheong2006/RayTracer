...
bool RayTracer::initialize(glm::ivec2 resolution, Type type) {
    ...
    if (type == Type::RGB) {
        m_shader->attachShaderCode(GL_FRAGMENT_SHADER, RGBRayTracerFragShaderSource + RayScene::genShaderCode());
    }
    else if (type == Type::Spectral) {
        m_shader->attachShaderCode(GL_FRAGMENT_SHADER, SpectralRayTracerFragShaderSource + RayScene::genShaderCode());
    }
    else {
